/**
 * parse from pagefind-web-js
 * @see https://github.com/CloudCannon/pagefind/blob/452398009b1243bc759455b9a02e22561a04bf0b/pagefind_web_js/types/index.d.ts#L87-L103
 */
declare global {
...
/**
 * assign from `pagefind-script.astro`
 */
declare global {
  interface Window {
    pagefind?: {
...
export type PagefindSearchData = PagefindSearchFragment & {
  id: string;
};

export async function pagefindSearch(
  value: string,
  options?: PagefindSearchOptions & { maxResults?: number },
): Promise<PagefindSearchData[]> {
  if (!window.pagefind) {
    console.warn('pagefind.js is not fully loaded.');
    console.warn('Please run `bun run build` and `bun run dev:copy:pagefind`.');
    return [];
  }

  const pagefindResults = await window.pagefind.search(value, options);
  ...
  const dataArray = await Promise.all(
    pagefindResults.results
      ...
      .map(async (result) => {
        ...
        return { id: result.id, ...data } satisfies PagefindSearchData;
      }),
  );
  return dataArray;
}
