...
export const PostPreview = async (slug) => {
const fileContent = await readFile(`./src/content/post/ko/writing/${slug}`, 'utf8');
const { data, content } = matter(fileContent);
const wordCount = content.split(' ').filter(Boolean).length;

return <section className="rounded-md bg-gray-100 px-4 py-2">

  <h5 className="font-bold">
    <a href={`/post/${slug.replace('.mdx', '')}`} target="_blank">{data.title}</a>
  </h5>
  <i>{wordCount} 단어</i>
</section> }

{await PostPreview("svelte-compiler-operation.mdx")}
...
<div>
{await (async() => {
const writing = await readdir('./src/content/post/ko/writing', { withFileTypes: true });
const dirs = writing.filter((file) => file.name.endsWith(".mdx"));
const res = await Promise.all(dirs.map((dir) => PostPreview(dir.name)))

return <div className="mb-4 flex h-72 flex-col gap-2 overflow-y-scroll">{res}</div>
})()}
