...
음악에서 [샘플링 기법](https://namu.wiki/w/%EB%94%94%EC%A7%80%ED%84%B8#s-3.1)을 활용하여 디지털의 불연속적인 간극을 메울 수 있듯이, 웹에서는 css의 [transition](https://developer.mozilla.org/en-US/docs/Web/CSS/transition)을 활용해 동떨어진 두 상태를 연결할 수 있습니다.
...
https://easings.net/ 에서 easing 그래프에 대해서 살펴보고 그래프의 청사진을 기억하면 이러한 디테일을 활용하는데 도움이 될 것입니다.
...
wwdc에서도 관련된 발표내용이 있는 것을 볼 수 있습니다.
...
좋은 맛을 내기 위해서, **신선한 눈으로 되돌아보는 것** 또한 중요합니다.
반복해서 애니메이션을 접하게 되면 익숙해져 실제보다 빠르게 동작을 인지하게 됩니다. 자신에게 잠시 쉼을 주고 나중에 다시 살펴봐보세요!

## Framer Motion

웹에서 애니메이션을 다루는 방법은 여러가지가 있으나 [framer-motion](https://www.framer.com/motion/)을 가장 추천합니다.
...
`layout`이라는 속성을 추가해주는 것 만으로 `width`, `position`, `flexbox` 등 CSS Animation으론 다룰 수 없는 애니메이션을 만들 수 있습니다.
...
무겁고 JS 기반으로 애니메이션을 구동하기에,

Framer Motion으로 구현 가능한 모든 것은 vanilla CSS, JS로도 구현할 수 있습니다.
...
- [Web Animation API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API/Web_Animations_API_Concepts)
- [Motion One](https://motion.dev/) for non-React projects

### demo

[layout and shared layout animations](https://www.framer.com/motion/layout-animations/)
